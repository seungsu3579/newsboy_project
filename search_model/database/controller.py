...
class ServiceDBController:
    ...
    def get_total_num_to_summarize(self):
        try:
            sql = "SELECT count(id) FROM news WHERE is_summary = false;"
    ...
    def get_news_to_summarize(self, data):
        try:
            ...
            sql = f"SELECT id, article, image_url, headline, article_url, created_date_time FROM news WHERE created_date_time between %s and %s LIMIT 800"
    ...
    def insert_keyword_news_relation(self, data: list):
        try:
            sql = """
            INSERT INTO news_keyword (created_date_time, last_modified_date_time, keyword_id, news_id) VALUES (%s, %s, %s, %s)
            """
    ...
    def update_article_context(self, data:list):
        try:
            sql = """
            UPDATE news SET article = %s WHERE id = %s
            """
            self.cur.executemany(sql, data)
            self.conn.commit()
        except Exception as e:
